...
{  
	...
	cout<<"\nTax Included: $"<< (totalPrice*0.0875)<<endl;
    cout << "\nTotal Overall Price: $" << resultPrice <<endl;
    
    cout<<"\nWe hope you enjoyed your meal at Brax Menner!"<<endl;
    cout<<"Would you like to register for our newsletter?"<<endl;
    cout<<"1 - Yes\n"<<"2 - No\n";
    cin>>menuChoice;
    if(menuChoice == 1){
        fileWrite(getUserRegistration(), CUST_FILE);
        cout <<"Registration successful, we hope you enjoyed!\n" <<endl;
    }
    else
    ...
    for(int i = 0; i <= Cookies; i = static_cast<Desserts>(i+1))
    {
        switch(i)
        {
            case 0:
                ...
                cout << "(Green Tea, Strawberry, Mint, Cookies and Cream, Chocolate, Vanilla)\n" << endl;
                break;
            case 1:
                cout << "2 - Cake";
                cout << setw(20) << setfill('.') << setw(25) << "$" << fixed << setprecision(2) << priceArray[i] << endl;
            ...
            case 6:
                cout << "7 - Donuts";
                cout << setw(20) << setfill('.') << setw(25) << "$" << fixed << setprecision(2) << priceArray[i] << endl;
                cout << "(Chocolate, Vanilla, Cinnamon, Powdered, Jelly)\n" << endl;
                break;
            case 7:
                cout << "8 - Cupcakes";
                cout << setw(20) << setfill('.') << setw(25) << "$" << fixed << setprecision(2) << priceArray[i] << endl;
            ...
            case 9:
                cout << "10 - Cookies";
                cout << setw(20) << setfill('.') << setw(25) << "$" << fixed << setprecision(2) << priceArray[i] << endl;
                cout << "(Chocolate Chip, Butter, Sugar, Oatmeal Raisin)\n" << endl;
                break;
            default:
...
User getUserRegistration()
{
    ...
    cout<<"Please enter the price you have paid: ";
...
void fileWrite (User currentUser, string textFile)
{
    ...
    customerFile.open(textFile.c_str(), fstream::out | fstream::app);
     
    customerFile <<currentUser.person <<endl;
    customerFile <<currentUser.password <<endl;
    customerFile <<currentUser.email <<endl;
    customerFile <<currentUser.resultPrice <<endl;
     
    customerFile.close();
}

/*
In program, always set variables equal to 0. To prevent random number from occupying said variable. Or else program will die.
